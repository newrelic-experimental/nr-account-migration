...
# Also specify the target account and target user API key (if the same user has access to both accounts, this value may be the same)
...
def setup_params():
    ...
    parser.add_argument('--synthetics', dest='synthetics', required=False, action='store_true', help='Pass --synthetics to migrate Synthetic monitor entity tags')
    parser.add_argument('--securecreds', dest='securecreds', required=False, action='store_true', help='Pass --securecreds to migrate Synthetic secure credentials entity tags')
    parser.add_argument('--apm', dest='apm', required=False, action='store_true', help='Pass --apm to migrate APM entity tags')
    parser.add_argument('--browser', dest='browser', required=False, action='store_true', help='Pass --browser to migrate Browser entity tags')
    parser.add_argument('--dashboards', dest='dashboards', required=False, action='store_true', help='Pass --dashboards to migrate dashboard entity tags')
    parser.add_argument('--infrahost', dest='infrahost', required=False, action='store_true', help='Pass --infrahost to migrate Infrastructure host entity tags')
    parser.add_argument('--infraint', dest='infraint', required=False, action='store_true', help='Pass --infraint to migrate Infrastructure integration entity tags')
    parser.add_argument('--mobile', dest='mobile', required=False, action='store_true', help='Pass --mobile to migrate Mobile entity tags')
    parser.add_argument('--lambda', dest='lambda_function', required=False, action='store_true', help='Pass --lambda to migrate Lambda function entity tags')
    return parser
...
def migrate_tags(from_file, src_account_id, src_api_key, tgt_account_id, tgt_api_key, entity_types):
    tag_status = {}
    ...
    for entity_name in entity_names:
        ...
        for entity_type in entity_types:
            if src_entity == None:
                src_result = ec.gql_get_matching_entity_by_name(src_api_key, entity_type, entity_name, src_account_id)
                if src_result['entityFound']:
                    ...
                    logger.debug("No match found in source account for entity with name " + entity_name + " with type " + entity_type + ". Continuing to check other defined entity types")
        if src_entity == None: 
            logger.error("Skipping entity. Unable to find entity " + entity_name + " in source account " + str(src_account_id))
            tag_status[entity_name][tgkeys.SOURCE_FOUND] = False
            continue
        tgt_entity = None
        for entity_type in entity_types:
            if tgt_entity == None:
                tgt_result = ec.gql_get_matching_entity_by_name(tgt_api_key, entity_type, entity_name, tgt_account_id)
                if tgt_result['entityFound']:
                    tgt_entity = tgt_result['entity']
                    tag_status[entity_name][tgkeys.ENTITY_TYPE] = entity_type
                    tag_status[entity_name][tgkeys.TARGET_FOUND] = True
                else:
                    logger.debug("No match found in target account for entity with name " + entity_name + " with type " + entity_type + ". Continuing to check other defined entity types")
        if tgt_entity == None:
            logger.error("Skipping entity. Unable to find entity " + entity_name + " in target account " + str(tgt_account_id))
        ...
        result = ec.gql_mutate_add_tags(tgt_api_key, tgt_entity['guid'], tags_needed)
        if 'error' in result:
...
def main():
    ...
    migrate_tags(args.fromFile[0], args.sourceAccount[0], source_api_key, args.targetAccount[0], target_api_key, entity_types)
